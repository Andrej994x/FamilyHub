...
[ApiController]
[Authorize]
[Route("api/families/{familyId:guid}/vault/warranties")]
public class WarrantiesController : ApiControllerBase
{
    private readonly IWarrantyService _service;
    private readonly IValidator<CreateWarrantyRequest> _createValidator;
    private readonly IValidator<UpdateWarrantyRequest> _updateValidator;

    public WarrantiesController(
        IWarrantyService service,
        IValidator<CreateWarrantyRequest> createValidator,
        IValidator<UpdateWarrantyRequest> updateValidator)
    {
        _service = service;
    ...
    [ProducesResponseType(typeof(WarrantyResponse), StatusCodes.Status201Created)]
    public async Task<IActionResult> Create(Guid familyId, [FromForm] CreateWarrantyRequest request)
    {
    ...
    [HttpGet]
    [ProducesResponseType(typeof(IReadOnlyList<WarrantyResponse>), StatusCodes.Status200OK)]
    ...
    [Consumes("multipart/form-data")]
    [ProducesResponseType(typeof(WarrantyResponse), StatusCodes.Status200OK)]
    public async Task<IActionResult> Update(Guid familyId, Guid id, [FromForm] UpdateWarrantyRequest request)
    {
